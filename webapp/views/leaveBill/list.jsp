 	...
 	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
		  ...
		  <tr>
		        <td height="20" bgcolor="#FFFFFF" class="STYLE10" colspan="8"><div align="left">
					<a href="/leaveBill/input">添加请假申请</a>
				</div></td>
		  </tr> 
		  <tr>
		    <td><table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#a8c7ce" onmouseover="changeto()"  onmouseout="changeback()">
		      ...
		      <c:if test="${list!=null &&list.size()>0}">
		      	<c:forEach items="${list}" var="list" varStatus="varCount">
		      		<tr>
				        <td height="20" bgcolor="#FFFFFF" class="STYLE6"><div align="center">${list.id}</div></td>
				        <td height="20" bgcolor="#FFFFFF" class="STYLE19"><div align="center">${list.user.name}</div></td>
				        ...
				        <td height="20" bgcolor="#FFFFFF" class="STYLE19"><div align="center">${list.leaveDate}</div></td>
				        ...
				        <c:choose>
				        	<c:when test="${list.state==0}">
			        			<a href="/leaveBill/input?id=${list.id}" >修改</a>
								<a href="/leaveBill/delete?id=${list.id}" >删除</a>
								<a href="/workflow/startProcess?id=${list.id}" >申请请假</a>
			        		</c:when>
			 				<c:when test="${list.state==1}">
			 					<a href="/workflow/his?id=${list.id}" >查看审核记录</a>
			 				</c:when>
			 				<c:otherwise>
			 					<a href="/leaveBill/delete?id=${list.id}" >删除</a>
			 					<a href="/workflow/his?id=${list.id}" >查看审核记录</a>
